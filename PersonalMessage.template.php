...
<form class="flow_hidden" action="', $scripturl, '?action=pm;sa=pmactions;', $context['display_mode'] == 2 ? 'conversation;' : '', 'f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', '" method="post" accept-charset="', $context['character_set'], '" name="pmFolder">';
	...
	if ($context['get_pmessage']('message', true))
	{
		...
		while ($message = $context['get_pmessage']('message'))
		{
			...
			echo '
	<div class="', $window_class, ' clear post_wrapper phoneVflow">
		...
		<div class="poster">
			...
			<div class="avatar">
				<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
					', !empty($message['member']['avatar']['image']) ? $message['member']['avatar']['image'] : $settings['sr_default_avatar'], '
				</a>
			</div>';
			...
			if (!$message['member']['is_guest'])
			{
				...
				echo '
				<li class="warning">', $context['can_issue_warning'] ? '<a href="' . $scripturl . '?action=profile;area=issuewarning;u=' . $message['member']['id'] . '">' : '', '<i class="fas fa-exclamation-circle"></i>    ', $context['can_issue_warning'] ? '</a>' : '', '<span class="warn_', $message['member']['warning_status'], '">', $txt['warn_' . $message['member']['warning_status']], '</span></li>';
			}
		...
		// SideMenu Button for Responsiveness.
		echo'
			...
			if ($context['can_send_pm'])
			{
				...
				if (!$message['member']['is_guest'])
				{
					...
					if ($message['number_recipients'] > 1 && $context['display_mode'] != 2)
						echo '
					<li class="reply_all_button"><a class="tooltip" title="', $txt['reply_to_all'], '" href="', $scripturl, '?action=pm;sa=send;f=', $context['folder'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';pmsg=', $message['id'], ';quote;u=all"><span>', $txt['reply_to_all'], '</span></a></li>';

					echo '
					<li class="reply_button"><a class="tooltip" title="', $txt['reply'], '" href="', $scripturl, '?action=pm;sa=send;f=', $context['folder'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';pmsg=', $message['id'], ';u=', $message['member']['id'], '"><span>', $txt['reply'], '</span></a></li>
					<li class="quote_button"><a class="tooltip" title="', $txt['quote'], '" href="', $scripturl, '?action=pm;sa=send;f=', $context['folder'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';pmsg=', $message['id'], ';quote', $context['folder'] == 'sent' ? '' : ';u=' . $message['member']['id'], '"><span>', $txt['quote'], '</span></a></li>';
				}
				// This is for "forwarding" - even if the member is gone.
				else
					echo '
					<li class="forward_button"><a class="tooltip" title="', $txt['reply_quote'], '" href="', $scripturl, '?action=pm;sa=send;f=', $context['folder'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';pmsg=', $message['id'], ';quote"><span>', $txt['reply_quote'], '</span></a></li>';
			}
			echo '
					<li class="remove_button"><a class="tooltip" title="', $txt['delete'], '" href="', $scripturl, '?action=pm;sa=pmactions;pm_actions[', $message['id'], ']=delete;f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', addslashes($txt['remove_message']), '?\');"><span>', $txt['delete'], '</span></a></li>';
	...
	<div class="pagesection">
		<div class="floatleft pagelinks">', $txt['pages'], ': ', $context['page_index'], '</div>
		<div class="floatright"><input type="submit" name="del_selected" value="', $txt['quickmod_delete_selected'], '" onclick="if (!confirm(\'', $txt['delete_selected_confirm'], '\')) return false;" class="button_submit" /></div>
	</div>';
...
echo'<script>jq(document).ready(function () {jq(window).resize(papaQuickButans);
			papaQuickButans();
		});</script>';
}
...
function template_subject_list()
{
	...
	$context['page_index'] = str_replace("[", "", str_replace("]", "", $context['page_index']));
	echo '
	<table width="100%" class="table_grid">
	<thead>
		<tr class="catbg">
			<th align="center" width="4%" class="first_th phoneNone">
			...
			<th class="lefttext phoneNone" width="22%">
	...
	while ($message = $context['get_pmessage']('subject'))
	{
		...
		<tr class="', $next_alternate ? 'windowbg' : 'windowbg2', '">
			<td align="center" width="4%" class="phoneNone">
			<script type="text/javascript"><!-- // --><![CDATA[
		...
		echo '
			...
			<td class="phoneNone">', $message['time'], '</td>
			<td>', ($context['display_mode'] != 0 && $context['current_pm'] == $message['id'] ? '<img src="' . $settings['images_url'] . '/selected.gif" alt="*" />' : ''), '<a href="', ($context['display_mode'] == 0 || $context['current_pm'] == $message['id'] ? '' : ($scripturl . '?action=pm;pmid=' . $message['id'] . ';kstart;f=' . $context['folder'] . ';start=' . $context['start'] . ';sort=' . $context['sort_by'] . ($context['sort_direction'] == 'up' ? ';' : ';desc') . ($context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : ''))), '#msg', $message['id'], '">', $message['subject'], '</a>', $message['is_unread'] ? '&nbsp;<img src="' . $settings['lang_images_url'] . '/new.gif" alt="' . $txt['new'] . '" />' : '', '<br /><span class="phoneOnly">', $message['time'], '</span></td>
