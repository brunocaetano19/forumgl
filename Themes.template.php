...
function template_set_settings()
{
	...
	foreach($context['menu_buttons'] as $act=>$button) {
		if($act!="logout" && $act!="login" && $act!="register")
			$mButtons[] = array(
				...
				'type' => 'text',
				'value' => !empty($settings[$act.'_icon']) ? $settings[$act.'_icon'] : (empty($menu_icons[$act]) ? 'fa-chevron-circle-right' : $menu_icons[$act]),
			);
	}
	$mButtons_three = array(
		array(
			'id' => 'logout_icon',
			'label' => 'Logout',
			'description' => $txt['menu_buttons_settings'],
			'type' => 'text',
			'value' => empty($menu_icons['logout']) ? 'fa-chevron-circle-right' : $menu_icons['logout'],
		),
		array(
			'id' => 'login_icon',
			'label' => 'Login',
			'description' => $txt['menu_buttons_settings'],
			'type' => 'text',
			'value' => empty($menu_icons['login']) ? 'fa-chevron-circle-right' : $menu_icons['login'],
		),
		array(
			'id' => 'register_icon',
			'label' => 'Register',
			'description' => $txt['menu_buttons_settings'],
			'type' => 'text',
			'value' => empty($menu_icons['register']) ? 'fa-chevron-circle-right' : $menu_icons['register'],
		),
	);
	$mButtons = array_merge($mButtons, $mButtons_three);
	$context['settings'] = array_merge($context['settings'], $mButtons);
	...
	foreach ($context['settings'] as $setting)
	{
		...
		elseif (is_string($setting))
			echo '<div class="sr_section_title">', $setting, '</div>';
		// A checkbox?
           			...
           			echo '
              			<br />
							...
							echo'
            					...
            					<dd>
              						<textarea style="width: 100%;box-sizing: border-box;" rows="4" cols="40" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '">', (empty($setting['value']) && isset($setting['empty'])) ? $setting['empty'] : $setting['value'], '</textarea>
            					</dd>';
		}
	...
	if (function_exists('sycho_credits'))
		sycho_credits();
