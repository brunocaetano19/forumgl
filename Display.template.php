...
function template_main()
{
	...
	if (!empty($context['topic_starter_id']))
	{
		$memID = $context['topic_starter_id'];
		loadMemberData($memID);
		loadMemberContext($memID, true);

		$ava = !empty($memberContext[$memID]['avatar']['image']) ? $memberContext[$memID]['avatar']['image'] : $settings['sr_default_avatar'];
	}
	else
		...
		<div class="threadthis">
			<div class="content">
				...
				<div class="threadthis_details">
					...
					<div class="detailS">
						' . fontawesome('fas fa-user') . '    ', (!empty($context['topic_starter_id']) && $context['topic_starter_id']!=0) ? '<a href="'.$scripturl.'?action=profile;u='.$memberContext[$memID]['id'].'">'.$memberContext[$memID]['name'].'</a>' : $txt['guest'], ' &middot;
						' . fontawesome('fas fa-comments') . '    ', $context['total_visible_posts'], ' &middot; ' . fontawesome('fas fa-eye') . '    ', $context['num_views'], '<div class="nextlinks">', $context['previous_next'], '</div>
					</div>
				</div>
			</div>
		</div>';
	...
	if ($context['is_poll'])
	{
		...
		template_button_strip_special($poll_buttons);
	...
	if (!empty($context['linked_calendar_events']))
	{
		...
		foreach ($context['linked_calendar_events'] as $event)
			echo '
							<li>
								', ($event['can_edit'] ? '<a href="' . $event['modify_href'] . '"> <img src="' . $settings['images_url'] . '/icons/modify_small.gif" alt="" title="' . $txt['modify'] . '" class="edit_event" /></a> ' : ''), '<strong>', $event['title'], '</strong>: ', $event['start_date'],
	...
	while ($message = $context['get_message']())
	{
		...
		if (!empty($message['member']['group']))
			echo '
								<li class="membergroup" ', !empty($message['member']['group_color']) ? 'style="color: '.$message['member']['group_color'].'"' : '', '>', $message['member']['group'], '</li>';
		...
		if (!$message['member']['is_guest'])
		{
			...
			if ((empty($settings['hide_post_group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
				echo '
								<li class="postgroup" ', !empty($message['member']['post_group_color']) ? 'style="color: '.$message['member']['post_group_color'].'"' : '', '>', $message['member']['post_group'], '</li>';
			echo '
								<li class="stars">', $message['member']['group_stars'], '</li>';
			echo'<ul class="greezeInsider">';
			// Show how many posts they have made.
			if (!isset($context['disabled_fields']['posts']))
				echo '
								<li class="postcount">' . fontawesome('fas fa-comments') . '    <strong class="sr_hidden">', $txt['member_postcount'], ':</strong> ', $message['member']['posts'], '</li>';

			// Is karma display enabled?  Total or +/-?
			if ($modSettings['karmaMode'] == '1')
				echo '
								<li class="karma">' . fontawesome('fas fa-heart') . '    <strong class="sr_hidden">', $modSettings['karmaLabel'], '</strong> ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</li>';
			elseif ($modSettings['karmaMode'] == '2')
				echo '
								<li class="karma">' . fontawesome('fas fa-theater-masks') . '    <strong class="sr_hidden">', $modSettings['karmaLabel'], '</strong> +', $message['member']['karma']['good'], '/-', $message['member']['karma']['bad'], '</li>';

			// Show the member's gender icon?
			if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '' && !isset($context['disabled_fields']['gender']))
				echo '
								<li class="gender"><i class="fas fa-', $message['member']['gender']['name']==$txt['male'] ? 'mars' : 'venus', '"></i>    <strong class="sr_hidden">', $txt['gender'], ':</strong>', $message['member']['gender']['name'], '</li>';

			// Is this user allowed to modify this member's karma?
			if ($message['member']['karma']['allow'])
				echo '
								<li class="karma_allow">
									<a href="', $scripturl, '?action=modifykarma;sa=applaud;uid=', $message['member']['id'], ';topic=', $context['current_topic'], '.' . $context['start'], ';m=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><span class="buttonLike icon_style tooltip" title="', $modSettings['karmaApplaudLabel'], '">' . fontawesome('fas fa-thumbs-up') . '</span></a>
									<a href="', $scripturl, '?action=modifykarma;sa=smite;uid=', $message['member']['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';m=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><span class="buttonLike icon_style tooltip" title="', $modSettings['karmaSmiteLabel'], '">' . fontawesome('fas fa-thumbs-down') . '</span></a>
								</li>';
			...
			if ($settings['show_profile_buttons'])
			{
				...
				if ($message['member']['can_view_profile'])
					echo '
										<li><a href="', $message['member']['href'], '">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/icons/profile_sm.gif" alt="' . $txt['view_profile'] . '" title="' . $txt['view_profile'] . '" class="tooltip"/>' : $txt['view_profile']), '</a></li>';

				// Don't show an icon if they haven't specified a website.
				if ($message['member']['website']['url'] != '' && !isset($context['disabled_fields']['website']))
					echo '
										<li><a href="', $message['member']['website']['url'], '" title="' . $message['member']['website']['title'] . '" target="_blank" class="new_win tooltip">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/www_sm.gif" alt="' . $message['member']['website']['title'] . '" />' : $txt['www']), '</a></li>';

				// Don't show the email address if they want it hidden.
				if (in_array($message['member']['show_email'], array('yes', 'yes_permission_override', 'no_through_forum')))
					echo '
										<li><a href="', $scripturl, '?action=emailuser;sa=email;msg=', $message['id'], '" rel="nofollow">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/email_sm.gif" alt="' . $txt['email'] . '" title="' . $txt['email'] . '" class="tooltip"/>' : $txt['email']), '</a></li>';
			...
			if ($message['member']['can_see_warning'])
				echo '
								<li class="warning"><span class="warn_', $message['member']['warning_status'], '">', $context['can_issue_warning'] ? '<a href="' . $scripturl . '?action=profile;area=issuewarning;u=' . $message['member']['id'] . '">' : '', '' . fontawesome('fas fa-exclamation-circle') . '    ', $context['can_issue_warning'] ? '</a>' : '', '', $txt['warn_' . $message['member']['warning_status']], '</span></li>';
		}
		...
		elseif (!empty($message['member']['email']) && in_array($message['member']['show_email'], array('yes', 'yes_permission_override', 'no_through_forum')))
			echo '
								<li class="email"><a href="', $scripturl, '?action=emailuser;sa=email;msg=', $message['id'], '" rel="nofollow">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/email_sm.gif" class="tooltip" alt="' . $txt['email'] . '" title="' . $txt['email'] . '" />' : $txt['email']), '</a></li>';
		...
		if ($context['can_moderate_forum'] && !empty($message['member']['ip']))
			echo '
									<br/><a href="', $scripturl, '?action=', !empty($message['member']['is_guest']) ? 'trackip' : 'profile;area=tracking;sa=ip;u=' . $message['member']['id'], ';searchip=', $message['member']['ip'], '">', $message['member']['ip'], '</a> <a href="', $scripturl, '?action=helpadmin;help=see_admin_ip" onclick="return reqWin(this.href);" class="help">(?)</a>';
		// Or, should we show it because this is you?
		elseif ($message['can_see_ip'])
			echo '
									<br/><a href="', $scripturl, '?action=helpadmin;help=see_member_ip" onclick="return reqWin(this.href);" class="help">', $message['member']['ip'], '</a>';
		...
		// SideMenu Button for Responsivenesse
		// If this is the first post, (#0) just say when it was posted - otherwise give the reply #.
		if ($message['can_approve'] || $context['can_reply'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'] || $context['can_report_moderator'] || $context['can_issue_warning'])
			echo '
		<div class="phoneListpapa">
			<div class="listOpener taphoOnlyInline floatright buttonLike icon_style" data-state="closed">' . fontawesome('fas fa-ellipsis-h') . '</div>
							<ul class="reset smalltext quickbuttons phoneList">';

		// Maybe we can approve it, maybe we should?
		if ($message['can_approve'])

							 echo '<li class="approve_button"><a class="tooltip" title="', $txt['approve'], '" href="', $scripturl, '?action=moderate;area=postmod;sa=approve;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><span>', $txt['approve'], '</span></a></li>';

		// Can they reply? Have they turned on quick reply?
		if ($context['can_quote'] && !empty($options['display_quick_reply']))

							 echo '<li class="quote_button"><a class="tooltip" title="', $txt['quote'], '" href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" onclick="return oQuickReply.quote(', $message['id'], ');"><span>', $txt['quote'], '</span></a></li>';

		// So... quick reply is off, but they *can* reply?
		elseif ($context['can_quote'])

							 echo '<li class="quote_button"><a class="tooltip" title="', $txt['quote'], '" href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '"><span>', $txt['quote'], '</span></a></li>';

		// Can the user modify the contents of this post?
		if ($message['can_modify'])

							 echo '<li class="modify_button"><a class="tooltip" title="', $txt['modify'], '" href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], '"><span>', $txt['modify'], '</span></a></li>';

		// How about... even... remove it entirely?!
		if ($message['can_remove'])

							 echo '<li class="remove_button"><a class="tooltip" title="', $txt['remove'], '" href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');"><span>', $txt['remove'], '</span></a></li>';

		// What about splitting it off the rest of the topic?
		if ($context['can_split'] && !empty($context['real_num_replies']))

							 echo '<li class="split_button"><a class="tooltip" title="', $txt['split'], '" href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '"><span>', $txt['split'], '</span></a></li>';

		// Can we restore topics?
		if ($context['can_restore_msg'])

							 echo '<li class="restore_button"><a class="tooltip" title="', $txt['restore_message'], '" href="', $scripturl, '?action=restoretopic;msgs=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><span>', $txt['restore_message'], '</span></a></li>';

		// Can the user modify the contents of this post?  Show the modify inline image.
		if ($message['can_modify'])
							 echo '<li class="can_quick_modify"><a class="tooltip" title="', $txt['quick_modify'], '" id="modify_button_', $message['id'], '" onclick="oQuickModify.modifyMsg(\'', $message['id'], '\')" >' . fontawesome('fas fa-pencil-alt') . '<span>', $txt['quick_modify'], '</span></a>';

		// Maybe they want to report this post to the moderator(s)?
		if ($context['can_report_moderator'])
							 echo '<li class="can_report_moderator"><a class="tooltip" title="', $txt['report_to_mod'], '" href="', $scripturl, '?action=reporttm;topic=', $context['current_topic'], '.', $message['counter'], ';msg=', $message['id'], '">' . fontawesome('fas fa-flag') . '<span>', $txt['report_to_mod'], '</span></a>';

		// Can we issue a warning because of this post?  Remember, we can't give guests warnings.
		if ($context['can_issue_warning'] && !$message['is_message_author'] && !$message['member']['is_guest'])
							 echo '<li class="can_issue_warning"><a class="tooltip" title="', $txt['issue_warning'], '" href="', $scripturl, '?action=profile;area=issuewarning;u=', $message['member']['id'], ';msg=', $message['id'], '">' . fontawesome('fas fa-exclamation-circle') . '<span>', $txt['issue_warning'], '</span></a>';


		// Show a checkbox for quick moderation?
		if (!empty($options['display_quick_mod']) && $options['display_quick_mod'] == 1 && $message['can_remove'])
							  echo '<li class="inline_mod_check" >
										<label class="container tooltip" title="', $txt['code_select'], '" id="in_topic_mod_check_', $message['id'], '" style="display: none;">
										</label>
									</li>';
	...
	//echo ' <div id="moderationbuttons">', template_button_strip_special($mod_buttons, 'bottom', array('id' => 'moderationbuttons_strip')), '</div>';

	// Show the page index... "Pages: [1]".
	echo '
			<div class="pagesection">
				', template_button_strip_special($normal_buttons, 'right'), '
				<div id="moderationbuttons">', template_button_strip_special($mod_buttons, 'bottom', array('id' => 'moderationbuttons_strip')), '</div>
